...
print(">>> Running version from ImferenceModel")
...
def visualize(rgb, detections, save_path="tmp.png"):
    ...
    img[mask] = alpha * np.array([r, g, b]) + (1 - alpha) * img[mask]
...
def batch_input_data(depth_path, cam_path, device):
    ...
    # Handle RGB depth image (convert to grayscale)
    if depth_raw.ndim == 3 and depth_raw.shape[2] == 3:
        print("Depth image has 3 channels. Converting to grayscale.")
        depth_raw = np.mean(depth_raw, axis=2)

    depth = np.array(depth_raw).astype(np.int32)  # Shape: [H, W]
    ...
    # #batch["depth"] = torch.from_numpy(depth).unsqueeze(0).to(device)
    # batch["depth"] = torch.from_numpy(depth).unsqueeze(0).unsqueeze(0).to(device)
...
def run_inference(segmentor_model, output_dir, cad_path, rgb_path, depth_path, cam_path, stability_score_thresh):
    ...
    if not os.path.exists(template_dir) or len(glob.glob(f"{template_dir}/*.npy")) == 0:
        print(f"No templates found, generating templates in: {template_dir}")
        ...
        # Run segmentation
        detections = model.segmentor_model.generate_masks(rgb_np)
        import gc
        torch.cuda.empty_cache()
        gc.collect()

        # Save original RGB image
        imageio.imwrite(os.path.join(template_dir, 'rgb_0.png'), rgb_np)

        # Save the first mask
        masks = detections["masks"]
        if len(masks) == 0:
            raise RuntimeError("No masks were returned by the segmentation model.")
        #binary_mask = masks[0].cpu().numpy().astype(np.uint8) * 255
        binary_mask = masks[0]
        if hasattr(binary_mask, 'cpu'):  # convert tensor to numpy
            binary_mask = binary_mask.cpu().numpy()
        binary_mask = (binary_mask > 0).astype(np.uint8) * 255

        imageio.imwrite(os.path.join(template_dir, 'mask_0.png'), binary_mask)

        # Save sampled 3D point cloud from CAD model
        cad_mesh = trimesh.load_mesh(cad_path, process=False, maintain_order=True)
        print("Bounding box extents (units unknown):", mesh.bounding_box.extents)

        if isinstance(cad_mesh, list):
            cad_mesh = cad_mesh[0]

        # Clean up just in case
        cad_mesh.visual.vertex_colors = None

        # Sample points
        #points = cad_mesh.sample(2048).astype(np.float32)
        points = cad_mesh.sample(2048).astype(np.float32) / 1000.0
        np.save(os.path.join(template_dir, 'xyz_0.npy'), points)

        print("TEMPLATE DIR:", template_dir)

    template_files = sorted(glob.glob(os.path.join(template_dir, "rgb_*.png")))
    MAX_TEMPLATES = 41  # Start small (e.g., 5) and increase if memory allows
    template_files = sorted(template_files)[:MAX_TEMPLATES]
    templates, masks, boxes = [], [], []

    for rgb_path in template_files:
        idx = osp.splitext(osp.basename(rgb_path))[0].split("_")[1]
        mask_path = os.path.join(template_dir, f"mask_{idx}.png")

        if not osp.exists(mask_path):
            print(f"[WARNING] Missing mask for template {idx}")
            continue
    ...
    # run inference
    rgb = Image.open(rgb_path).convert("RGB")
    #rgb = rgb.resize((320, 320), Image.BILINEAR)
